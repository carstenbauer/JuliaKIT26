...
3. When asked for it, input either `uc3-login1.scc.kit.edu` or `uc3-login2.scc.kit.edu` for the hostname and press `Enter`. (You should stick to one login node for the entire workshop.)
4. Enter your OTP and then your password.
5. Wait a bit...
...
2. In the terminal in the bottom, navigate to your copy of the workshop materials: `cd $HOME/JuliaKIT26`.
...
2. Inside of the workshop directory `$HOME/JuliaKIT26` (you should still be there), start Julia with `julia --project`.
...
## BONUS / OPTIONAL: Using the Julia VSCode extension on the cluster
...
2. Search for `julia` and install the extension (like you did locally on your laptop).
...
```
/pfs/work9/workspace/scratch/ka_rx8865-juliakit26/julia_wrapper.sh
```

To set the relevant setting:

1. Press `CTRL + ,` (comma) to open the Settings.
2. Select the tab (at the top) that says "uc3-login1.scc.kit.edu" or "uc3-login2.scc.kit.edu".

...
3. Finally, make sure that the code is working by running it and inspecting the final visualization.
  - For testing, you can use the script `job_script_testing.sh` (submit it via `qsub job_script.sh`).
...
1. Look at the file `job_script.sh` and then submit it as a job via `qsub job_script.sh`.

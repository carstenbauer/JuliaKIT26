...
3. Finally, make sure that the code is working by running it and inspecting the final visualization.
  - For testing, you can use the script `job_script_testing.sh` (submit it via `sbatch job_script.sh`).
  - If you're working interactively on a GPU **compute node** (within a SLURM allocation) you can run the file in an interactive REPL via `include("diffusion_2d_gpu.jl")` or from the command line via `julia --project diffusion_2d_gpu.jl`.

## Task 2: Benchmark

We want to assess the performance of our 2D diffusion solver on the GPU.

1. Look at the file `job_script.sh` and then submit it as a job via `sbatch job_script.sh`.
